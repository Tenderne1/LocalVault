...
impl VaultManager{
 ...
 pub fn backup(&mut self,destination:&str)->Result<(),String>{let _=self.dek.ok_or("Vault locked")?;if !self.path.exists(){return Err("Vault file does not exist".into())}fs::copy(&self.path,destination).map_err(|e|e.to_string())?;Ok(())}
