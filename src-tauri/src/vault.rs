...
pub struct UnlockSecurityState{
...
pub struct VaultManager{path:PathBuf,dek:Option<[u8;DEK_LEN]>,recovery_pending_dek:Option<[u8;DEK_LEN]>,captcha:Option<(String,String)>}
impl VaultManager{
 pub fn new()->Self{
  ...
  Self{path:base.join("vault.db"),dek:None,recovery_pending_dek:None,captcha:None}
}
 ...
 fn read_failure_times(&self,c:&Connection)->Vec<i64>{
   self.meta(c,"unlock_failures").ok()
     .and_then(|b|String::from_utf8(b).ok())
     .and_then(|s|serde_json::from_str::<Vec<i64>>(&s).ok())
     .unwrap_or_default()
     .into_iter().filter(|t|now_ms().saturating_sub(*t)<=UNLOCK_WINDOW_MS).collect()
 }
 fn write_failure_times(&self,c:&Connection,times:&[i64])->Result<(),String>{
   let data=serde_json::to_vec(times).map_err(|e|e.to_string())?;
 ...
 fn current_unlock_state(&mut self,c:&Connection)->Result<(Vec<i64>,Option<i64>),String>{
   ...
   let stored=self.meta(c,"unlock_lock_until").ok()
     .and_then(|b|String::from_utf8(b).ok())
     .and_then(|s|s.parse::<i64>().ok());
   let lock_until=stored.filter(|x|*x>now_ms());
   if stored.is_some()&&lock_until.is_none(){
 ...
 fn captcha_svg(code:&str)->String{
   let mut svg=String::from(r##"<svg xmlns="http://www.w3.org/2000/svg" width="240" height="84" viewBox="0 0 240 84"><rect width="240" height="84" rx="10" fill="#f4f6f8"/>"##);
   // Noise makes the challenge less suitable for blind OCR while remaining readable to a human.
   for i in 0..14{
     let a=Self::random::<2>();
     let x1=a[0] as u32%240; let y1=a[1] as u32%84;
     let x2=(a[0] as u32*37+i*19)%240; let y2=(a[1] as u32*29+i*11)%84;
     svg.push_str(&format!(r##"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="#8b95a1" stroke-opacity=".35" stroke-width="{}"/>"##,x1,y1,x2,y2,if i%3==0{2}else{1}));
   }
   for (i,ch) in code.chars().enumerate(){
     let a=Self::random::<2>(); let y=55+(a[1] as u32%12);
     let rot=(a[0] as i32%25)-12; let x=26+i*34;
     svg.push_str(&format!(r##"<text x="{}" y="{}" transform="rotate({} {} {})" font-family="Arial,sans-serif" font-size="34" font-weight="700" fill="#25313d">{}</text>"##,x,y,rot,x,y,ch));
   }
   svg.push_str("</svg>");svg
 }
 pub(crate) fn new_captcha(&mut self)->String{
   const ALPHABET:&[u8]=b"ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
   let r=Self::random::<6>();let mut code=String::with_capacity(6);
   for b in r{code.push(ALPHABET[(b as usize)%ALPHABET.len()] as char);}
   let svg=Self::captcha_svg(&code);self.captcha=Some((code,svg.clone()));svg
 }
   ...
   let result=(||{
     ...
     if times.len()>=CAPTCHA_AFTER_FAILURES{
       ...
       if captcha.unwrap_or("").trim().eq_ignore_ascii_case(&expected)==false{
         ...
         // Do not count it as a password failure: the password has not been checked yet.
         self.new_captcha();
         return Err("验证码错误，验证码已刷新，请重新输入".into())
       }
     }
     Ok(())
   })();
 ...
 pub fn unlock(&mut self,pass:&str,captcha:Option<&str>)->Result<Vec<Entry>,String>{
   ...
   let auth_result=(||{
     ...
     let mut d=[0u8;32];d.copy_from_slice(&raw);Ok(d)
   })();
   let mut d=match auth_result{
     Ok(d)=>d,
     Err(e)=>{let _=self.record_unlock_failure();let _=self.protect_storage();return Err(e)}
   };
   let result=(||{
     let c=self.db()?;self.clear_unlock_failures(&c)?;self.dek=Some(d);self.read_entries(&c,&d)
   })();
   d.zeroize();
 ...
 pub fn recovery_set_master(&mut self,new_password:&str,confirm:&str)->Result<(),String>{
   ...
   if result.is_ok(){let _=self.recovery_pending_dek.take();if let Some(mut old)=self.dek.take(){old.zeroize();}if let Ok(c)=self.db(){let _=self.clear_unlock_failures(&c);}}
   let _=self.protect_storage();
   result
 }
...
impl Drop for VaultManager{fn drop(&mut self){if let Some(mut d)=self.dek.take(){d.zeroize()}if let Some(mut d)=self.recovery_pending_dek.take(){d.zeroize()}self.captcha=None;let _=self.protect_storage();}}
