...
impl VaultManager{
 ...
 fn create_versioned_backup(&self,settings:&mut BackupSettings,force:bool)->Result<Option<PathBuf>,String>{
  ...
  let fingerprint=Self::fingerprint_entries(&entries);
 ...
 pub fn backup_now(&self)->Result<BackupSettings,String>{let mut settings=self.load_backup_settings();settings.enabled=true;match self.create_versioned_backup(&mut settings,true){Ok(_)=>{self.store_backup_settings(&settings)?;Ok(settings)},Err(e)=>{self.record_backup_error(&mut settings,e.clone());Err(e)}}}
 ...
 fn read_backup_file(&self,backup_path:&str,pass:&str)->Result<([u8;DEK_LEN],BackupData),String>{
   ...
   let mut st=c.prepare("SELECT name,icon,parent_name FROM categories ORDER BY CASE WHEN parent_name IS NULL THEN 0 ELSE 1 END, COALESCE(parent_name,'') ASC, sort_order ASC, created_at ASC").map_err(|e|e.to_string())?;
